...
angle_measure = repmat([0;1.3;-1.3],sum(param.active_leg),1);
...
next_pitch = tgt_pitch+1e-5;
next_roll = tgt_roll+1e-5;
...
for i=1:size(fix_foot_id_list,1)
    ...
    p_rf = autoFunc_fk_pf_pos(theta,param.rho_opt_true(:,i),param.rho_fix(:,i));
...
leg_v_estimation = zeros(3*param.num_leg,1);
for i = 1:param.num_leg
    ...
    leg_v_estimation((i-1)*3+1:(i-1)*3+3) = leg_v;
    r((i-1)*3+1:(i-1)*3+3) = body_v - leg_v;
end
% wrong calf length
wrong_r = zeros(3*param.num_leg,1);
wrong_leg_v_estimation = zeros(3*param.num_leg,1);
for i = 1:param.num_leg
    angle = joint_angle_list((i-1)*3+1:(i-1)*3+3);
    av = joint_av_list((i-1)*3+1:(i-1)*3+3);
    % get opt rho TODO: check dimension here
    rho_wrong = state(10+(i-1)*param.rho_opt_size+1:10+i*param.rho_opt_size,1) - 0.01;
    p_rf = autoFunc_fk_pf_pos(angle,rho_wrong,param.rho_fix(:,i));
    J_rf = autoFunc_d_fk_dt(angle,rho_wrong,param.rho_fix(:,i));

    leg_v = -R_er*(J_rf*av+skew(omega)*p_rf);
    wrong_leg_v_estimation((i-1)*3+1:(i-1)*3+3) = leg_v;
    wrong_r((i-1)*3+1:(i-1)*3+3) = body_v - leg_v;
end

disp('true velocity')
body_v
disp('velocity estimation using true leg length')
estimated_v = mean(reshape(leg_v_estimation,[3,4]),2)
disp('velocity estimation using true leg length - 0.01m')
wrong_estimated_v = mean(reshape(wrong_leg_v_estimation,[3,4]),2)
disp('average velocity estimation error with 0.01m leg length error')
error_v = mean(reshape(wrong_r,[3,4]),2)