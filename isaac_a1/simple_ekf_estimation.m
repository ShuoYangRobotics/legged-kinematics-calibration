...
% measurement noise covariance, only consider xy velocity
R = diag(0.001*ones(param.num_leg,1));
% save initial state
...
for i=2:traj_len
    ...
    % only measure xy velocity
    z = [vi(1)';vi(1)';vi(1)';vi(1)'];
    % the predict output
    ...
    if (i > 5)
        ...
        var_forces = 100*ones(param.num_leg,1);
    end
    for j = 1:param.num_leg
        no_contact = 1-1/(1+exp(-1*(foot_force.Data(i,j)-50)));
        contacts(j) = 1- no_contact;
        R(j,j) = 5000*no_contact + 5*abs(var_forces(j))^2 + 0.00001;
