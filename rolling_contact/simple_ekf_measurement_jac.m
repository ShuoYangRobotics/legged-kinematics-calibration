...
jac = zeros(param.num_leg*2, param.rho_opt_size*param.num_leg*2);    
...
for i = 1:param.num_leg
    ...
    dz_drho = -R_er*(kron(av',eye(3))*dJdrho+kron(drho',eye(3))*dfddrho+skew(omega)*dfdrho);
    dz_ddrho = -R_er*dfdrho;
    jac((i-1)*2+1:(i-1)*2+2,(i-1)*param.rho_opt_size+1:i*param.rho_opt_size) = dz_drho(1:2,:);
